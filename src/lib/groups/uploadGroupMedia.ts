...
import type { GroupPostMediaType } from "@/components/groups/types";
...
export type UploadedGroupMedia = {
  media_url: string;
...
export async function uploadGroupMedia(
  ...
  userId: string,
  files: File[]
): Promise<{ media: UploadedGroupMedia[]; error?: string }> {
  ...
  const media: UploadedGroupMedia[] = [];
  let sortOrder = 0;
  ...
  for (const file of files) {
    ...
    const path = `${groupId}/${userId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
