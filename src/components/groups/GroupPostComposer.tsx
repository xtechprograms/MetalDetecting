...
import { uploadGroupMedia } from "@/lib/groups/uploadGroupMedia";
...
export function GroupPostComposer({
  ...
  function addFiles(selected: FileList | null, kind: "image" | "video") {
    ...
    setPreviews((prev) => [...prev, ...incoming.map((file) => URL.createObjectURL(file))]);
    setError(null);
  }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const { media, error: uploadError } = await uploadGroupMedia(
      supabase,
      groupId,
      userId,
      files
    ...
    onPosted({ ...inserted, author: profile, media: savedMedia });
    setBody("");
    setFiles([]);
